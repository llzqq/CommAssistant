...
public:
    ...
    ///Returns true if the Comm Assistant bridge is active.
    bool isCommAssistantBridgeActive() const;

    ///Returns true if the active bridge/interface is a CAN connection.
    bool isActiveBridgeCan() const;

    ///Sends data through the active Comm Assistant bridge if available.
    bool sendThroughCommAssistant(const QByteArray& data, bool* success, QString* errorText);
...
public slots:
    ...
    void commAssistantBridgeStateChangedSlot(bool active, bool isCanMode);
    void commAssistantBridgePayloadReceivedSlot(QByteArray payload, bool isCanMode);
    void commAssistantBridgePayloadSentSlot(QByteArray payload, bool success, bool isCanMode);
    void updateUnifiedSendAvailabilitySlot(void);
...
private slots:
   ...
   ///Is called if the style shall be changed.
    ...
    ///Opens communication assistant.
    void showCommAssistantSlot(void);
