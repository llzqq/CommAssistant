...
#include "canTab.h"
#include "commassistant/commassistantdialog.h"
    ...
    m_scriptToolBoxPage(), m_closedByScript(false), m_exitCode(0), m_commAssistantDialog(0), m_commAssistantAction(0)
{
    ...
    m_commAssistantDialog = new CommAssistantDialog(this);
    connect(m_commAssistantDialog, SIGNAL(bridgeStateChanged(bool,bool)), this, SLOT(commAssistantBridgeStateChangedSlot(bool,bool)));
    connect(m_commAssistantDialog, SIGNAL(bridgePayloadReceived(QByteArray,bool)), this, SLOT(commAssistantBridgePayloadReceivedSlot(QByteArray,bool)));
    connect(m_commAssistantDialog, SIGNAL(bridgePayloadSent(QByteArray,bool,bool)), this, SLOT(commAssistantBridgePayloadSentSlot(QByteArray,bool,bool)));
    ...
    updateUnifiedSendAvailabilitySlot();
...
void MainWindow::sendButtonPressedSlot(void)
{
    ...
    const QString currentFormat = m_userInterface->SendFormatComboBox->currentText();
    ...
    else if(currentFormat.contains("can"))
    {
        QByteArray canData;

        quint8 type = m_userInterface->CanTypeBox->currentIndex();
        if(currentFormat == "can-fd")
        {
            type += PCAN_MESSAGE_FD;
        }
        else if(currentFormat == "can-fd brs")
        {
    ...
    if(isActiveBridgeCan() || m_mainInterface->isConnectedWithCan())
    {
      if(!currentFormat.contains("can"))
      {
        sendData.clear();
        QMessageBox::critical(this, "error", QString("The active interface is CAN, but the send format is not can or can-fd.")
                            .arg(currentFormat));
      }
    }
    else
    ...
    if(!sendData.isEmpty())
    {
      ...
      const bool bridgeActive = isCommAssistantBridgeActive();
      if(bridgeActive)
      {
          bool success = false;
          QString errorText;
          sendThroughCommAssistant(sendData, &success, &errorText);
          if(!success)
          {
              QMessageBox::critical(this, "error", QString("sending failed: %1").arg(errorText));
          }
    ...
    {
        QMessageBox::critical(this, "error", "sending failed: empty send data");
    }
}
...
bool MainWindow::isCommAssistantBridgeActive() const
{
    return (m_commAssistantDialog != 0) && m_commAssistantDialog->isBridgeActive();
}

bool MainWindow::isActiveBridgeCan() const
{
    return (m_commAssistantDialog != 0) && m_commAssistantDialog->isCanBridgeActive();
}

bool MainWindow::sendThroughCommAssistant(const QByteArray& data, bool* success, QString* errorText)
{
    const bool ok = (m_commAssistantDialog != 0) && m_commAssistantDialog->sendFromMainWindow(data, errorText);
    if(success != 0)
    {
        *success = ok;
    }
...
void MainWindow::commAssistantBridgeStateChangedSlot(bool active, bool isCanMode)
{
    Q_UNUSED(isCanMode)
    if(active)
    {
        m_statusBarLabel.setText(QString("Comm Assistant bridge active (%1)")
                                 .arg(isActiveBridgeCan() ? "CAN" : "data"));
    }
    else if(!m_isConnected)
    {
        m_statusBarLabel.setText("Comm Assistant bridge inactive");
    }
    updateUnifiedSendAvailabilitySlot();
}

void MainWindow::commAssistantBridgePayloadReceivedSlot(QByteArray payload, bool isCanMode)
{
    m_handleData->bridgeDataReceivedSlot(payload, isCanMode);
    showNumberOfReceivedAndSentBytes();
}

void MainWindow::commAssistantBridgePayloadSentSlot(QByteArray payload, bool success, bool isCanMode)
{
    if(!success)
    {
        return;
    ...
    m_handleData->bridgeDataSentSlot(payload, true, isCanMode);
    m_handleData->addDataToSendHistory(&payload);
    showNumberOfReceivedAndSentBytes();
}

void MainWindow::updateUnifiedSendAvailabilitySlot(void)
{
    m_sendWindow->setIsConnected(m_isConnected || isCommAssistantBridgeActive());
}
...
/**
 * Opens the communication assistant.
 */
void MainWindow::showCommAssistantSlot(void)
{
    if(m_commAssistantDialog == 0)
    {
        m_commAssistantDialog = new CommAssistantDialog(this);
    }

    m_commAssistantDialog->show();
    m_commAssistantDialog->showNormal();
    m_commAssistantDialog->raise();
    m_commAssistantDialog->activateWindow();
}
